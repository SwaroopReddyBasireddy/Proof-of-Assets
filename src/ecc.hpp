    ...
    while(i < numbits) {
        ...
        if (i == numbits-1) {
            ...
            twoadders.emplace_back(this->pb,
                outx, outy,
                (i == 0 ? 0 : accumx[accnext-1]),
                (i == 0 ? 0 : accumy[accnext-1]),
                svec[i],
              //  0, 0,
              //  twoiPx, twoiPy
                curveParams<FieldT>::Cx, curveParams<FieldT>::Cy,
         ...
         //   twoi1PCx = twoiPx;
         //   twoi1PCy = twoiPy;
         //   ec_double_point(twoi2PCx, twoi2PCy, twoiPx, twoiPy);
            ...
            i += 2;
          //  ec_double_point(twoiPx, twoiPy, twoi2PCx, twoi2PCy);
...
public:
    ...
    outx(outx), outy(outy), svec(svec), Px(Px), Py(Py)
  {
    ...
    scalarmuls.emplace_back(pb, outx, outy, svec, Px, Py);
...
public:
  ...
  //  this->pb.add_r1cs_constraint(r1cs_constraint<FieldT>(acciny, 1, curveParams<FieldT>::Ay));
    ...
    mulgadgets.emplace_back(this->pb, accmidx, accmidy, a, n1,
            curveParams<FieldT>::Gx, curveParams<FieldT>::Gy);
    mulgadgets.emplace_back(this->pb, accoutx, accouty, b, n2,
            curveParams<FieldT>::Hx, curveParams<FieldT>::Hy);
    // Subtract the accumulator excess to get the result
    addgadget.emplace_back(this->pb, outx, outy, accoutx, accouty, accmidx, accmidy);
  }
...
public:
  ...
  POA_gadget(protoboard<FieldT> &pb,
	...
	void generate_r1cs_witness() {
		...
		this->pb.val(t) = rand() % uint64_t(pow(2,51));
